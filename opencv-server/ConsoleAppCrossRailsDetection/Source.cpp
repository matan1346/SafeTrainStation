...
const char * SERVER_URL = "Your API server";
	...
	while (true) {
		...
		if (!safeState)
		{
			...
			if (dangerFrameCounter == 0) {
				...
				/* alert via http */
				//threadHttp = std::thread(httpAlert, SERVER_URL, "AlertWrapper", stationID, cameraID, timestamp);
				httpAlert.run(SERVER_URL, "AlertWrapper", stationID, cameraID, timestamp);
			}
		...
		if (dangerFrameCounter > 0 && frameCounter - dangerStart > 20)
		{
			...
			// alert via http
			httpAlert.run(SERVER_URL, "S3Wrapper", stationID, cameraID, timestamp);
